...
- `$A` = `launcher/plasma.android/android17` (the 153 GB AOSP checkout — **reference only, never built, never packaged, stays untracked**).
...
2. `Artboard/docs/requirements.md` (mandatory per `/implement_artboard` — every Artboard change below follows that skill's V-model).
3. `cosmo/linux_main.cpp` — the GTK3→CairoTarget host glue to copy.
4. `cosmo/touch/PhoneApp.{h,cpp}` — existing touch-style Artboard app (screens + animated transitions); reuse its patterns.
...
DM Sans: app-private Fontconfig registration in the host (`cosmo/linux_main.cpp` shows the
...
launcher/
  ...
  android-shell/
    ...
    tests/        (host-side golden tests; Artboard core tests stay in Artboard/tests)
```

`launcher/plasma.android/` gets a `.gitignore` entry (never committed, never packaged).
...
| ID | Change | HAL? | Draft requirement (paste into `Artboard/docs/requirements.md`) |
|---|---|---|---|
...
Rules that bind every AB task: tests in `Artboard/tests/coreTests.cpp` via RecordingTarget
...
`launcher/android-shell/theme+shell`, **not** in Artboard — Artboard only gains the six
...
- **Guards:** install rules must whitelist paths explicitly — `launcher/plasma.android/` (153 GB) and `$A` assets must be impossible to glob into a package.
