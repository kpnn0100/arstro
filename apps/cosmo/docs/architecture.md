...
| `core/ImageProcessing/src/color/ColorMixer.{h,cpp}` | engine | per-hue HSL adjustment. **No longer a `PointProcessor`** (R-MIXER-5): whether a pixel belongs to a colour is a question about its neighbours, so the stage owns a two-pass loop — a weight plane per non-flat channel, softened, then larger-magnitude-wins against the pixel's own answer. `spread == 0` collapses back to the point op exactly |
| `apps/cosmo/segment/OnnxSegmenter.{h,cpp}` | host | a REAL segmentation model behind `ISegmenter` (R-AISEG-15). Host layer because it reads a file and loads a shared library, neither of which `cosmo_core` may do. Links **nothing** — ONNX Runtime is found with `LoadLibrary`/`dlopen` through its one exported C symbol, the `OmpPin` pattern — so a machine with no runtime builds and runs identically. Driven by a text manifest beside the model; declines every subject the manifest does not declare |
| `apps/cosmo/third_party/onnxruntime_c_api.h` | host | vendored, MIT. The struct's field order IS the ABI, so a hand-written subset would compile and call the wrong function pointer |
| `core/ImageProcessing/src/engine/MaskStack.{h,cpp}` | engine | mask coverage + the blend, for all five types. `maskLoops` is the one place that says which polygons a mask is made of — a `Path`'s flattened outline or a `Detect` mask's found regions — so the fill, the point query and the on-photo stroke are looking at the same shapes. Since R-AISEG-21 it takes no segmenter and produces no outlines: a render decides nothing about WHERE a mask is |
| `core/ImageProcessing/src/analysis/Segmenter.{h,cpp}` | engine | **how strongly each pixel is SKIN** (R-AISEG-22/23). deepgaze's two-stage colour detector: a range gate seeding a hue/saturation back-projection fitted to the pixels of this photograph. Not a network — the library carries no assets and opens no files. Produces a LIKELIHOOD, never a region: thresholding and cleanup are `Detection`'s, so an installed model gets the same treatment. `ISegmenter` is the seam a host with a real model installs |
| `core/ImageProcessing/src/analysis/Detection.{h,cpp}` | engine | **the detection, as an operation** (R-AISEG-19..24): `preparing -> colour -> regions -> shapes -> outline` over the framed, UNADJUSTED photo, returning loops in normalised coordinates plus a progress callback. Runs once when asked, never inside a render, and what it returns becomes the mask |
| `core/ImageProcessing/src/analysis/Contour.{h,cpp}` | engine | a plane of numbers -> geometry: `traceCoverageOutline` (marching squares, closing regions along the frame edge) and `simplifyLoop` (Douglas-Peucker). Lifted out of `MaskStack.cpp` when the render stopped tracing — it knows nothing about masks |
