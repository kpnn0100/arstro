...
| `core/ImageProcessing/src/engine/MaskStack.{h,cpp}` | engine | mask coverage + the blend, and `traceCoverageOutline` — the marching-squares tracer that turns a COMPUTED mask's coverage into drawable geometry (R-AISEG-13). The outline rides on `RenderService::Frame` beside the histograms, because it is a product of one render and only true of that render |
| `core/ImageProcessing/src/analysis/Segmenter.{h,cpp}` | engine | **which pixels are sky / skin / foliage / water / hair** (R-AISEG). A per-class statistical model — colour, position, local structure — NOT a network: the library carries no assets and opens no files. `ISegmenter` is the seam a host with a real model installs on `EditEngine`; the built-in is the fallback for whatever it declines |
