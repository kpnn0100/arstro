...
| `core/ImageProcessing/src/color/ColorMixer.{h,cpp}` | engine | per-hue adjustment: Hue and Sat through HSL, **Lum as a GAIN on the pixel** (R-MIXER-10..13 — Adobe's `value scale`, which for a uniform multiply is the same thing, so hue and HSV saturation are preserved by construction). **No longer a `PointProcessor`** (R-MIXER-5): whether a pixel belongs to a colour is a question about its neighbours, so the stage owns a two-pass loop — a weight plane per non-flat channel, softened, then larger-magnitude-wins against the pixel's own answer. `spread == 0` collapses back to the point op exactly |
...
| `core/ImageProcessing/src/engine/MaskStack.{h,cpp}` | engine | mask coverage + the blend, for all four types |
