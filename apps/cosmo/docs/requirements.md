...
`apps/cosmo/segment/OnnxSegmenter`, the vendored ONNX Runtime C header, the model-fetch script and
the model survey, `MaskParams::Semantic` with its `subject`/`sensitivity`/`regions`, the
`mask detect` command, the three `detect.*` events, `AppModel::detect`, `AppModel::segmenter`,
`RenderService`'s detect surface and segmenter seam, `EditEngine::detectSubject`, the Detect chip
and its panel block, and the on-photo boundary the overlay drew for it. See R-AISEG in
`REQUIREMENTS.md` for why, and `git log --grep AISEG` for the three attempts.

**What survives, and it is the only part that matters now.** `MaskParams::Type` value **4 is not
reused**, and the mask blob parser stays tolerant of what a project written while the feature
existed contains: thirteen numbers in the first group instead of eleven, and one trailing group
per found region. Both are skipped rather than rejected — the first group has always been read by
index and stopped at eleven, and any group after the path has always been ignored — so such a
project loads and its Detect mask renders as no coverage, which is what this format has always
done with a mask type it does not know. Re-pointing 4 at a live type would turn a mask that does
nothing into a mask that does something wrong.

`Mask_roundtrips_through_the_project_and_the_preset` (`image_tests`) asserts exactly that: a blob
