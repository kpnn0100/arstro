 ...
 *
 *  A 9-voice synth sounding 9 notes at once is rendered into a buffer (NOT timed —
 *  R-DSP-3), and only the effect chain that follows it is measured: compressor ->
 *  EQ (high-pass + low-pass) -> reverb, over 48000 samples per channel at 48 kHz.
 *
...
namespace arstrobench
{
    ...
    public:
        ...
        static constexpr int kFrames = 48000;  ///< R-DSP-2: one second at 48 kHz
        static constexpr int kPasses = 5;      ///< R-SCORE-3 (a DSP pass is short; sample more)
        ...
        /** Render the 9-voice chord into one buffer per channel. Public so a test can
         *  assert the synth actually produced signal before the chain is measured. */
        std::vector<std::vector<Sample>> generate() const;
