...
- [x] **M1.1** Scaffold `apps/launcher/android-shell/` dir tree (plan §3.4) + root `CMakeLists.txt`, added
  ...
  → Done: `apps/launcher/android-shell/{CMakeLists.txt, shell/main.cpp}` + §3.4 subdir stubs; wired into
...
- [x] **M1.2** `shell/SurfaceHost` — wrap one layer-shell toplevel + its `CairoTarget` + one Artboard
  ...
  mode for goldens. Copy the GTK3→Cairo glue from `apps/cosmo/linux_main.cpp`.
...
- [x] **M4.3** Swipe-to-dismiss physics + heads-up (auto-dismiss, drag-to-open) + DND + empty state.
- [x] **M4.4** L0 replay tests for dismissal, L1 goldens, L2 in isolated `dbus-run-session`.
...
- 2026-07-24 — M2.3: the generated icon header (`GeneratedIcons.h`) lives in the **build dir**
  (`build/apps/launcher/android-shell/gen/theme/icons/`), NOT committed — the committed source of truth is
...
- 2026-07-24 — `apps/launcher/` tracked inside the umbrella `arstro` repo (android17 gitignored) rather
